...
"""
import unittest
from stemmadist.dists.graphs.graph_edit import \
...
class TestGraphEditDistance(unittest.TestCase):
    ...
    def test_graph_distance_different_structure(self):
        ...
        distance = graph_distance(self.tree_1, self.tree_3)
    ...
    def test_graph_distance_node_insertion_deletion(self):
        ...
        distance = graph_distance(self.tree_1, self.tree_3)
        expected_min_distance = 2  # At least two edit operations expected
        self.assertGreaterEqual(distance, expected_min_distance)

    def test_graph_edit_distance_function(self):
        """Test main graph_edit_distance function with tree strings."""
        edit_distance = graph_edit_distance(self.tree_1_str, self.tree_2_str)
        # Expecting a distance of 1 due to one substitution
        self.assertEqual(edit_distance, 1)

        # Test with different structure
        edit_distance_diff_structure = graph_edit_distance(self.tree_1_str,
                                                           self.tree_3_str)
        self.assertGreater(edit_distance_diff_structure, 1)
